...
**Preview servers (don't ask, just do):** the Claude preview registry is shared across sessions, so `portal` (3001) is often already taken by another session. If it's taken (`preview_start("portal")` → `reused:true`, the page keeps navigating away, or another session is driving it), **immediately start `portal-b` (3005), then `portal-c` (3006)** from `.claude/launch.json` instead. Never fight over 3001 or report a busy port as a blocker. Cookies are not port-scoped on `localhost`, so an existing :3001 login carries over to :3005/:3006, no re-login. Same pattern for other apps: add a `-b`/`-c` config if you need a second instance.
...
4. **Anti-Anglicism** (full mappings: `C:\Users\karam\Documents\Claude\Projects\EINS\context\05-niche-dictionary.md`):
