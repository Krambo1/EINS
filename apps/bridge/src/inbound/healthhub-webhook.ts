...
import { db, getLinkByClinicAndVendor } from "../db/client.js";
 ...
 *
 * HealthHub signs outbound deliveries with a per-subscription secret using
 * HMAC-SHA256. We persist that secret in pvs_link.connection_config
 * .outbound_secret during the subscription POST handshake (see
 * adapters/healthhub/subscription.ts).
 *
 * On success: events are POSTed to the portal and we return 200.
 * On HMAC failure: 401 (HealthHub will alert the partner contact).
...
export async function healthHubWebhook(
  ...
  const events = healthHubAdapter.decodePush!(link, rawBody, req.headers as Record<string, string>);
